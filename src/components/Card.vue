<template>
    ...
    <div class="flex md:flex-row flex-col mt-2">
        <button :disabled="!userStore.user" @click="writeReview(item.id, type === 'upcoming' ? 'film' : type)"
        ...
        <button :disabled="!userStore.user"
            @click="addToFavourite(item.id, nome, type === 'upcoming' ? 'film' : type, item.backdrop_path, item.vote_average)"
            class=" flex rounded-lg bg-indigo-400
                    text-indigo-600 p-2 justify-center w-full">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
                class="w-5 h-5">
                <path stroke-linecap="round" stroke-linejoin="round"
